...
export default new vuex.Store({
  ...
  getters: {
	...
	getResultMovies: state => {
		console.log(state.result)
      let Arrays = new Array()
      for (let x = 0; x < state.movies.length; x++) {
        for (let y = 0; y < state.result.length; y++) {
          if (state.movies[x].id === state.result[y]) {
            Arrays.push(state.movies[x])
          }
        }
      }
  ...
  mutations: {
	...
	getResult: state => {
      ...
      axios
        .get('/api/jwt/movie/searchresult.do')
        .then(function(respone) {
          state.result = respone.data.data
        })
        .catch(function(error) {
          localStorage.removeItem('token', '')
        })
